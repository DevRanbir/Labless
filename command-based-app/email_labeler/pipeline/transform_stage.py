...
# Delay between LLM calls (seconds) to avoid rate-limit 429 errors.
# Groq free tier allows ~30 req/min on llama-3.1-8b-instant, so ~2 s is safe.
LLM_INTER_REQUEST_DELAY: float = 2.0
...
class TransformStage(PipelineStage):
    ...
    def execute(
        ...
        for i, email in enumerate(input_data):
            # Throttle requests to avoid rate-limit errors (429).
            # Skip the delay before the very first request.
            if i > 0 and not context.preview_mode and not context.dry_run:
                time.sleep(LLM_INTER_REQUEST_DELAY)
