...
class ExtractStage(PipelineStage):
    ...
    def _extract_from_gmail(self, context: PipelineContext) -> List[EmailRecord]:
        ...
        # If max_results is None, fetch all matching emails from Gmail.
        limit = self.config.max_results if self.config.max_results is not None else None
        raw_emails = self.email_processor.fetch_emails_from_gmail(
            query=self.config.gmail_query, limit=limit
        )
